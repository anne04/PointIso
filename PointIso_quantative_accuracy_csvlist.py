...
###########
choice = 1 #(1 for human, 2 for potato, 3 for background)
mappath='/data/fzohora/dilution_series_syn_pep/LC_MS/'
datapath='/data/fzohora/dilution_series_syn_pep/' 
peaks_filename ='/data/fzohora/dilution_series_syn_pep/feature_list/PEAKs/PEAKSX/'+dataname[test_index]+'_peptide_feature.csv'
openMS_file=datapath+'feature_list/openMS/'+dataname[test_index]+'_openMS_features'   
...
#    print(dataname[test_index])
    ...
    for i in range (1, len(peptide_mascot)):
        if float(peptide_mascot[i][4])<=threshold_score:
            continue
        if round(float(peptide_mascot[i][7]), 2) < min_RT:
            continue
    ...
    feature_table,auc_list=pickle.load(f)
        f.close()  
    pointIso_feature_list_file='/home/fzohora/bsi/feature_list_csv_'+dataname[test_index]+'_feature_list_brief.csv'
    feature_list=[]
    with open(pointIso_feature_list_file) as file:
    ...
    for i in range (1, len(feature_list)):
         # m/z = [RT peak, charge, intensity, score, total isotopes]
        feature_table[np.float(feature_list[i][0])].append([[np.float(feature_list[i][1]), np.float(feature_list[i][2]), np.float(feature_list[i][5], np.float(feature_list[i][6]), np.float(feature_list[i][7])]])    
        ...
        for j in range (0, len(mz_range)):
            ...
            if mz in feature_table:
                ...
                for k in range (0, len(ftr_list)):
                    ...
                    ftr_z=int(ftr[1])    
                    peak_RT=ftr[0]
                    score_ftr=np.max(ftr[3])
                    if (np.argmax(ftr[3])==1 and score_ftr<.80) or (ftr[4]==1 and score_ftr<.50) or score_ftr<.30: 
                        continue
                    if (round(float(peptide_mascot[i][7])-RT_tolerance, 2) <= peak_RT) and (peak_RT<=round(float(peptide_mascot[i][7])+RT_tolerance, 2)) and ftr_z==int(peptide_mascot[i][3]):
                        ...
                        if peptide_mascot[i][8]=='potato':
                            potato_detected_dict[peptide_mascot[i][5]]='found'
                            pointIso_potato_peptide_quantity[pep_seq_key].append(ftr[2])
                        elif peptide_mascot[i][8]=='human':
                            human_detected_dict[peptide_mascot[i][5]]='found'
                            pointIso_human_peptide_quantity[pep_seq_key].append(ftr[2])
                        else:
                            background_detected_dict[peptide_mascot[i][5]]='found'
                            pointIso_background_peptide_quantity[pep_seq_key].append(ftr[2])
...
#        filename ='/data/fzohora/dilution_series_syn_pep/feature_list/dino/'+dataname[test_index]+'_dino.csv'
        dino_peptide_mascot = []
        try:
            dino_filename ='/data/fzohora/dilution_series_syn_pep/feature_list/dino/'+dataname[test_index]+'.features.tsv'
            # reading csv file
            csvfile=open(dino_filename, 'r')
            # creating a csv reader object
...
#        
    ...
    ######################################################################
        MQ_filename ='/data/fzohora/dilution_series_syn_pep/feature_list/maxQ/'+dataname[test_index]+'_3.csv'
         
        # initializing the titles and peptide_mascot list
        MQ_peptide_mascot = [] 
        # reading csv file
        csvfile=open(MQ_filename, 'r')
        # creating a csv reader object
        csvreader = csv.reader(csvfile)     
        # extracting each data row one by one
        for row in csvreader:
...
#        
    ...
    ########################################################################
        ...
        f=open(openMS_file, 'rb') # open('/media/fzohora/USB20FD/raw/mzml/'+dataname[test_index]+'_openMS_features', 'rb')
