...
#IMP FINDING: MZ=0 POINTS CAN'T BE IGNORED
...
log_no='deepIso_pointNet_isoDetect_mz5_v3_'+'lrp001r2' #  'deepIso_pointNet_isoDetect_extendedWeight_nb2_'
momentum_value=0.90
reg_weight=0.001
total_frames_var=20
RT_window=15
...
with tf.device('/device:GPU:'+ sys.argv[4]): #):  # 
    ...
    P_W_conv3 = weight_variable([1, 1 , 128, 32], 'P_W_conv3')# #20x193
    P_b_conv3 = bias_variable([32], 'P_b_conv3') #for each of feature maps    

    nw_W_out = weight_variable([1, 1 , 32, num_class], 'nw_W_out')
    ...
    for row_idx in range (1, 4):
        ...
        #    T_net_mlp_2 = tf.tanh(conv2d(T_net_mlp_1, T_net_W_conv2) + T_net_b_conv2) # now the layer is : b x n x 1024
            ...
            nw_mlp_0 = tf.reshape(nw_mlp_0,[-1,datapoints,16])        
            # ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    #    total_loss= tf.reduce_mean(tf.reduce_sum(considered_loss, axis=1) / tf.to_float(tf.reduce_sum(sample_weight,axis=1))) #tf.reduce_mean(tf.reduce_sum(considered_loss, axis=1) / tf.to_float(tf.math.count_nonzero(sample_weight,axis=1)))
    #    model_optimizer=tf.train.RMSPropOptimizer(learn_rate) #tf.train.AdamOptimizer(learn_rate)
    #    
    #mat_diff=tf.constant(np.eye(transformation_matrix_holder.get_shape()[1].value), dtype=tf.float32) - tf.matmul(transformation_matrix_holder, tf.transpose(transformation_matrix_holder, perm=[0,2,1]))
    #L_reg=tf.nn.l2_loss(mat_diff)* reg_weight
    #total_loss = total_loss+L_reg
    ...
    #        train_step = model_optimizer.minimize(total_loss)
...
#                        break
    ...
    print('writing')
    f=open(datapath+'LC_MS/'+dataname[data_index]+'_pointNet_seg_list_dict_mz5_'+sys.argv[3], 'wb') 
    pickle.dump([list_dict,float(sys.argv[1])], f, protocol=2) #all mz_done
    f.close()
    print('done')
