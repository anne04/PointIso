...
log_no='deepIsoV2_isoGrouping_auc_exact_v2_r4' #r1 is w/o fp, r2 wat w/o TB 
batch_size=128
...
with tf.device('/gpu:'+ gpu):  #sys.argv[1]): # 
    ...
    # pool - 3, 1
#
#    W_conv2 = weight_variable([2, 1 , 16, 32], 'W_conv2')# 3,1 # done
#    b_conv2 = bias_variable([32], 'b_conv2') #for each of feature maps
    

#    W_conv3 = weight_variable([2, 1, 8, 16], 'W_conv3')  # 2,1
#    b_conv3 = bias_variable([16], 'b_conv3') 
    ...
    #2 x 1
#    W_fr_0 = weight_variable([1, ,  3 * 1 * 16 + 8 , 4], 'W_fr_0') # + 4
#    b_fr_0 = bias_variable([16], 'b_fr_0')
...
##saver.restore(sess, modelpath+'init-model_'+log_no_old+'.ckpt')
...
#saver.restore(sess, modelpath+'trained-model_'+log_no+'_best_loss_save_1.ckpt')
#
saver.restore(sess, modelpath+'trained-model_'+log_no+'_best.ckpt')
#saver.save(sess, modelpath+'trained-model_'+log_no+'_best_save_1.ckpt')

#
#saver.restore(sess, modelpath+'trained-model_'+log_no+'_before_best.ckpt')


##########################

#################### report feature module#####################################
#for val_index in (44,23,28,31,34,40,48,52,56):
#for val_index in range(44, 45): 
discard_list=[23,28,31,34,40,44,48,52,56]
#for val_index in (23,28,31,34,40,44,48,52,56):
for val_index in range(9,10): 
#for val_index in range(0, len(dataname)):
#    if val_index in discard_list:
#        continue
#    if val_index == 9:
#        continue
    print(dataname[val_index]) 
    f=open(mappath+dataname[val_index]+'_pointNet_v6r1_clusters_mz3_v5', 'rb') # 98.35    
#    f=open(mappath+dataname[val_index]+'_pointNet_v3r2_withFP_clusters_mz3_v4', 'rb') # 98.35
    ...
    f=open(datapath+'feature_list/'+'pointCloud_'+dataname[val_index]+'_ms1_record_mz5', 'rb')
    ...
    f=open(datapath+'feature_list/mascot/'+dataname[val_index]+'_mascot_db_search', 'rb')
    ...
    for i in range (1, len(peptide_mascot)):
        if float(peptide_mascot[i][4])<=threshold_score:
            continue
        if round(float(peptide_mascot[i][4]), 2) < 10.26 or round(float(peptide_mascot[i][2]), mz_resolution)>=(2000-0.50) or round(float(peptide_mascot[i][2]), mz_resolution)<400: #or round(float(peptide_mascot[i][2]), mz_resolution)>800 or 
            continue
    ...
    for i in range (1, len(peptide_mascot)):
        found=0
        mz_exact=round(float(peptide_mascot[i][2]), 2)
        total_feature=total_feature+1           
        mz_range=[]
        mz_range.append(mz_exact)    
        tolerance_mz=0.01 #(mz_exact*10.0)/10**6  # dinosaur = 0.005 
        mz_range.append(round(mz_exact-tolerance_mz, 2))
        mz_range.append(round(mz_exact+tolerance_mz, 2))
    ...
    f=open(datapath+'/feature_list/deepIsoV2_'+dataname[val_index]+'_featureTable_v6r1_cv5_ev2r4','wb') #'_featureTable_v3r4', 'wb')
...
#                                ftr_pred=copy.deepcopy(ftr)
    ...
    ###############
    #
    #f=open(datapath+'/feature_list/deepIsoV2_'+dataname[val_index]+'_featureTable', 'rb')
...
#        f=open(datapath+'/feature_list/deepIsoV2_'+dataname[val_index]+'_featureTable_v6r1_cv5_ev2r1_merged','wb') #'_featureTable_v3r4_merged', 'wb')
#        pickle.dump(feature_table, f, protocol=2)
#        f.close() 

    
#    f=open(datapath+'/feature_list/deepIsoV2_'+dataname[val_index]+'_featureTable_v3r2_cv4_exact_merged','wb') #'_featureTable_v3r4_merged', 'wb')
#    pickle.dump(feature_table, f, protocol=2)
#    f.close() 
    ...
    #                    feature_table[mz][k][len(ftr)-1].append(100)
                        break
